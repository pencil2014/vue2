<template>
	<div class="ex-display">
		...
		<div class="ex-button">
			<button @click="save" :class="{disable:disableBtn}">提交</button>
		</div>
		...
		getDetail () {
			...
			axios.post('commodityInfo/get',qs.stringify({
				...
				if (res.data.code === '10000') {
					...
					_this.typeName = _this.$route.query.typeName
					_this.groupId = _this.$route.query.groupId*1
					let Arr = res.data.data.commodityAffixEntityList
					Arr.forEach(function(v,k){
						_this.imgurl.push(v.filePath)
						_this.imgArray.push(v.filePath)
					})
				} else {
					Toast(res.data.msg)
				}
...
.ex-display .wrapper .item .report-file span{cursor: pointer;display: inline-block;width: 100%;line-height: 20px;padding-top: 12px;vertical-align: top;}
...
.ex-display .wrapper .item .UpLoadIMG div{display: inline-block;width: 64px;height: 64px;margin: 0 9px;}
...
.ex-display .wrapper .item .UpLoadIMG{width: 100%;}
